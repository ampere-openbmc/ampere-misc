    ...
    while (size)
    {
        ...
        if (i != result)
        {
            ...
            log_printf(LOG_ERROR,
                       ...
                       " written to flash\n",
                       written, written + i, argv2_ptr, written + result,
                       filestat.st_size);
            ret = -1;
            goto out;
        }
 ...
 * @params  mtd_fd [IN] - File descriptor of SPI NOR device contains NVPARAM
 *partition nvparam_base [IN] - Base offset of NVPARAM partition nvparam_blob
 *[OUT] - Buffer of read NVPARAM
 * @return  0 - Success
 * 			-1 - Failure
 *--------------------------------------------------------------------------*/
...
int main(int argc, char* argv[])
{
    ...
    int options_used[MAX_OPTIONS] = {
        0, 0, 0, 0, 0, 0, 0, 0, 0}; /* c, d, f, h, o, r, s, l, e */
    char* filepath = NULL;
    ...
    if (options_used[OPTION_L])
    {
        ...
        /* List out all valid NVPARAM (CRC field is identical with calculated
         * CRC) */
        for (index = 0; index < max_items; index++)
        {
            ...
            if (tmp_crc16 == cal_crc16)
            {
                log_printf(
                    ...
                    nvparam_base + (index * sizeof(struct nvparam_entry)),
                    blob[index].param1, blob[index].acl_rd, blob[index].acl_wr);
                found_records++;
            }
        }
