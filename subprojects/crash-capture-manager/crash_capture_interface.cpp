	...
	numericSensorEventSignal = std::make_unique<sdbusplus::bus::match_t>(
		...
		[&](sdbusplus::message::message &msg) {
			try {
                 ...
                 */
				msg.read(tid, sensorId, eventState,
					 preEventState, sensorDataSize,
					 presentReading);
                 ...
                 */
				if (sensorId == 175) {
					handleBertHostBootEvent(tid, sensorId,
								presentReading);
				}
			} catch (const std::exception &e) {
				std::cerr
					<< "handleNumericSensorEventSignal failed\n"
					<< e.what() << std::endl;
			}
		});
}
...
void CrashCapture::handleBertHostOnEvent(void)
{
	...
	bertHostFailTimer->start(
		std::chrono::milliseconds(BERT_HOSTFAIL_TIMEOUT));
	checkBertFlag = true;
