...
static int handshakeSPIHandler(sdbusplus::bus::bus& bus, bert_host_state state)
{
    ...
    for (int i = 0; i < bertMaxIndex; i++)
    {
        std::cerr << "BERT_PARTITION_INFO size = " << bertInfo.files[i].size
                  << "\n";
        std::cerr << "BERT_PARTITION_INFO name = " << bertInfo.files[i].name
                  << "\n";
    ...
    for (i = 0; i < bertMaxIndex; i++)
    {
        ...
        if (!ret)
        {
            ...
            if (!out.is_open())
            {
                error("Can not open ofstream for {VALUE}", "VALUE",
                      bertDumpPath.c_str());
                continue;
            }
            ...
            std::filesystem::copy(
                bertDumpPath.c_str(), faultLogFilePath.c_str(),
...
int bertHandler(sdbusplus::bus::bus& bus, bert_host_state state)
{
    ...
    {
        error("Failed bertHandler. ERROR = {ERR_EXCEP}", "ERR_EXCEP", e.what());
        return -1;
    }
