...
ErrorData errorTypeTable[] = {
    ...
    {0, error_mem_ue, "error_mem_ue", TYPE_MEM, UE_MEM_IERR, "UE_Memory_IErr",
     "MemoryECCUncorrectable"},
    {0, error_pcie_ue, "error_pcie_ue", TYPE_PCIE, UE_PCIE_IERR, "UE_PCIE_IErr",
    ...
    {1, error_core_ue, "error_core_ue", TYPE_CORE, UE_CORE_IERR,
     "UE_CPU_IError", "CPUError"},
    {1, error_mem_ue, "error_mem_ue", TYPE_MEM, UE_MEM_IERR, "UE_Memory_IErr",
     "MemoryECCUncorrectable"},
    {1, error_pcie_ue, "error_pcie_ue", TYPE_PCIE, UE_PCIE_IERR, "UE_PCIE_IErr",
     "PCIeFatalUncorrectableInternal"},
    ...
    {0, error_mem_ce, "error_mem_ce", TYPE_MEM, CE_MEM_IERR, "CE_Memory_IErr",
     "MemoryECCCorrectable"},
    {0, error_pcie_ce, "error_pcie_ce", TYPE_PCIE, CE_PCIE_IERR, "CE_PCIE_IErr",
     "PCIeFatalECRCError"},
    ...
    {1, error_core_ce, "error_core_ce", TYPE_CORE, CE_CORE_IERR,
     "CE_CPU_IError", "CPUError"},
    {1, error_mem_ce, "error_mem_ce", TYPE_MEM, CE_MEM_IERR, "CE_Memory_IErr",
     "MemoryECCCorrectable"},
    {1, error_pcie_ce, "error_pcie_ce", TYPE_PCIE, CE_PCIE_IERR, "CE_PCIE_IErr",
     "PCIeFatalECRCError"},
    {1, error_other_ce, "error_other_ce", TYPE_OTHER, CE_OTHER_IERR,
     "CE_SoC_IErr", "AmpereCritical"},
    {0, error_smpro, "error_smpro", TYPE_SMPM, SMPRO_IERR, "SMPRO_IErr",
     "AmpereCritical"},
    {0, error_pmpro, "error_pmpro", TYPE_SMPM, PMPRO_IERR, "PMPRO_IErr",
     "AmpereCritical"},
    {1, error_smpro, "error_smpro", TYPE_SMPM, SMPRO_IERR, "SMPRO_IErr",
     "AmpereCritical"},
    {1, error_pmpro, "error_pmpro", TYPE_SMPM, PMPRO_IERR, "PMPRO_IErr",
     "AmpereCritical"},
    {0, warn_smpro, "warn_smpro", TYPE_SMPM, SMPRO_IERR, "SMPRO_IErr",
     "AmpereCritical"},
    {0, warn_pmpro, "warn_pmpro", TYPE_SMPM, PMPRO_IERR, "PMPRO_IErr",
     "AmpereCritical"},
    {1, warn_smpro, "warn_smpro", TYPE_SMPM, SMPRO_IERR, "SMPRO_IErr",
     "AmpereCritical"},
    {1, warn_pmpro, "warn_pmpro", TYPE_SMPM, PMPRO_IERR, "PMPRO_IErr",
     "AmpereCritical"},
};
...
EventData eventTypeTable[] = {
    {0, 0, event_vrd_warn_fault, "event_vrd_warn_fault", TYPE_STATE,
     STATUS_READ_TYPE, S0_VRD_WARN_FAULT, "VR_WarnFault", "AmpereWarning"},
    {1, 0, event_vrd_hot, "event_vrd_hot", TYPE_TEMP, TEMP_READ_TYPE,
     S0_VRD_HOT, "VR_HOT", "AmpereWarning"},
    {2, 0, event_dimm_hot, "event_dimm_hot", TYPE_TEMP, TEMP_READ_TYPE,
     S0_DIMM_HOT, "DIMM_HOT", "AmpereWarning"},
    {3, 1, event_vrd_warn_fault, "event_vrd_warn_fault", TYPE_STATE,
     STATUS_READ_TYPE, S1_VRD_WARN_FAULT, "VR_WarnFault", "AmpereWarning"},
    {4, 1, event_vrd_hot, "event_vrd_hot", TYPE_TEMP, TEMP_READ_TYPE,
     S1_VRD_HOT, "VR_HOT", "AmpereWarning"},
    {5, 1, event_dimm_hot, "event_dimm_hot", TYPE_TEMP, TEMP_READ_TYPE,
     S1_DIMM_HOT, "DIMM_HOT", "AmpereWarning"},
    {6, 0, event_dimm_2x_refresh, "event_dimm_2x_refresh", TYPE_MEM,
     STATUS_READ_TYPE, S0_DIMM_2X_REFRESSH, "DIMM_2X_REFRESH_RATE",
     "AmpereWarning"},
...
{
    std::vector<uint8_t> eventData(ampere::sel::SEL_OEM_DATA_MAX_SIZE, 0xFF);
    ...
    eventData[6] = ((data.socket & 0x1) << 7) | ((eFields.subType & 0x7) << 4) |
...
static int logInternalErrorToRedfish(ErrorData data, InternalFields eFields)
{
    ...
    {
        snprintf(
            sErrorCode, MAX_MSG_LEN, "%s",
            ampere::internalErrors::errorCodes[eFields.errCode].description);
    }
    ...
    snprintf(redfishComp, MAX_MSG_LEN, "S%d_%s: %s %s %s with", data.socket,
             data.errName, sImage, sDir, sLocation);
    ...
    if (data.intErrorType == error_smpro || data.intErrorType == error_pmpro ||
        data.intErrorType == warn_smpro || data.intErrorType == warn_pmpro)
    {
        sd_journal_send("REDFISH_MESSAGE_ID=%s", redfishMsgID,
                        "REDFISH_MESSAGE_ARGS=%s,%s", redfishComp, redfishMsg,
                        NULL);
    }
...
static int parseAndLogInternalErrors(ErrorData data, std::string errLine)
{
    ...
    if (data.intErrorType == error_smpro || data.intErrorType == warn_smpro)
    {
...
static int logErrorToRedfish(ErrorData data, ErrorFields eFields)
{
    ...
    if (mapOfOccur.size() != 0 && mapOfOccur.count(temp) > 0)
    {
        ...
        {
            snprintf(redFishMsg, MAX_MSG_LEN, eInfo.errMsgFormat, str1, str2);
        }
    ...
    if (apiIdx == error_core_ue || apiIdx == error_core_ce)
    {
        ...
        snprintf(sTemp, MAX_MSG_LEN, "%s: %s %s", data.errName, redFishComp,
                 redFishMsg);
        sd_journal_send("REDFISH_MESSAGE_ID=%s", redFishMsgID,
    ...
    else if (apiIdx == error_mem_ue || apiIdx == error_mem_ce)
    {
        ...
        {
            sd_journal_send("REDFISH_MESSAGE_ID=%s", redFishMsgID,
                            "REDFISH_MESSAGE_ARGS=%d,%s,%d,%d", socket, dimCh,
    ...
    {
        sd_journal_send("REDFISH_MESSAGE_ID=%s", redFishMsgID,
                        "REDFISH_MESSAGE_ARGS=%d,%d,%d", socket, inst_13_0, 0,
                        NULL);
    }
...
static void getErrorsAndEvents()
{
    ...
    for (index = 0; index < NUMBER_OF_ERRORS; index++)
    {
        ...
        filePath = ampere::utils::getAbsolutePath(data1.socket, data1.label);
        if (filePath != "")
        {
    ...
    for (index = 0; index < NUMBER_OF_EVENTS; index++)
    {
        ...
        filePath = ampere::utils::getAbsolutePath(data2.socket, data2.label);
        if (filePath != "")
        {
