...
static unsigned int ShiftData(char* data, unsigned int* result, int len)
{
    ...
    if (bit_count != len)
    {
        printf("[%s] Expected Data Length is [%d] but not [%d] ", __func__,
               bit_count, len);
                           ...
                           int ufm_size, int ufm0_size, int endcfg_size)
{
    ...
    while (NULL != fgets(tmp_buf, ReadLineSize, jed_fd))
    {
        ...
        if (CFStart)
        {
                ...
                strlen(tmp_buf) != 1)
            {
                if (startWith(tmp_buf, "0") || startWith(tmp_buf, "1"))
                {
                    ...
                    ShiftData(data_buf, &dev_info->CF[current_addr],
                              LATTICE_COL_SIZE);

#ifdef VERBOSE_DEBUG
                    //          printf("[%d]%x %x %x %x\n",dev_info->CF_Line,
                    //          dev_info->CF[current_addr],dev_info->CF[current_addr+1],dev_info->CF[current_addr+2],dev_info->CF[current_addr+3]);
                    printf("[%d] ", dev_info->CF_Line);
                    for (i = 0; i < sizeof(unsigned int); i++)
                    {
                        printf("%x ", (dev_info->CF[current_addr + i]) & 0xff);
                        printf("%x ",
                               (dev_info->CF[current_addr + i] >> 8) & 0xff);
                        printf("%x ",
                               (dev_info->CF[current_addr + i] >> 16) & 0xff);
                        printf("%x ",
                               (dev_info->CF[current_addr + i] >> 24) & 0xff);
                    }
                    printf("\n");
#endif
                    ...
                    for (i = 0; i < sizeof(unsigned int); i++)
                    {
                        JED_CheckSum += (dev_info->CF[current_addr + i] >> 24) &
                                        0xff;
                        JED_CheckSum += (dev_info->CF[current_addr + i] >> 16) &
                                        0xff;
                        JED_CheckSum += (dev_info->CF[current_addr + i] >> 8) &
                                        0xff;
        ...
        {
            if (!startWith(tmp_buf, TAG_USERCODE /*"NOTE User Electronic"*/) &&
                strlen(tmp_buf) != 1)
            {
                VersionStart = 0;
                ...
                {
                    copy_size = indexof(tmp_buf, "*") - indexof(tmp_buf, "H") -
                                1;
        ...
        else if (UFMStart)
        {
                ...
                !startWith(tmp_buf, "L") && strlen(tmp_buf) != 1)
            {
                if (startWith(tmp_buf, "0") || startWith(tmp_buf, "1"))
                {
                    ...
                    ShiftData(data_buf, &dev_info->UFM[current_addr],
                              LATTICE_COL_SIZE);
#ifdef VERBOSE_DEBUG
                    ...
                    {
                        JED_CheckSum +=
                            (dev_info->UFM[current_addr + i] >> 24) & 0xff;
                        JED_CheckSum +=
                            (dev_info->UFM[current_addr + i] >> 16) & 0xff;
                        JED_CheckSum += (dev_info->UFM[current_addr + i] >> 8) &
                                        0xff;
                        JED_CheckSum += (dev_info->UFM[current_addr + i]) &
                                        0xff;
                    }
        ...
        else if (CFEnd)
        {
                ...
                !startWith(tmp_buf, "L") && strlen(tmp_buf) != 1)
            {
                if (startWith(tmp_buf, "0") || startWith(tmp_buf, "1"))
                {
                    current_addr = (dev_info->EndCF_Line * LATTICE_COL_SIZE) /
                                   32;
                    memset(data_buf, 0, sizeof(data_buf));
                    memcpy(data_buf, tmp_buf, LATTICE_COL_SIZE);
                    ShiftData(data_buf, &dev_info->EndCF[current_addr],
                              LATTICE_COL_SIZE);
#ifdef VERBOSE_DEBUG
                    printf("dev_info->EndCF_Line : %d | %x %x %x %x\n",
                           dev_info->EndCF_Line, dev_info->EndCF[current_addr],
                           dev_info->EndCF[current_addr + 1],
                           dev_info->EndCF[current_addr + 2],
                           dev_info->EndCF[current_addr + 3]);
#endif
                    // each data has 128bits(4*unsigned int), so the for-loop
                    // need to be run 4 times
                    for (i = 0; i < sizeof(unsigned int); i++)
                    {
                        JED_CheckSum +=
                            (dev_info->EndCF[current_addr + i] >> 24) & 0xff;
                        JED_CheckSum +=
                            (dev_info->EndCF[current_addr + i] >> 16) & 0xff;
                        JED_CheckSum +=
                            (dev_info->EndCF[current_addr + i] >> 8) & 0xff;
                        JED_CheckSum += (dev_info->EndCF[current_addr + i]) &
                                        0xff;
                    }
...
static int jtag_sendCFdata(CPLDInfo* dev_info)
{
    ...
    {
        printf("Writing Data: %d/%d (%.2f%%) \r", (i + 1), dev_info->CF_Line,
...
static int jtag_cpld_verify(CPLDInfo* dev_info)
{
    ...
    for (i = 0; i < dev_info->CF_Line; i++)
    {
        printf("Verify Data: %d/%d (%.2f%%) \r", (i + 1), dev_info->CF_Line,
               (((i + 1) / (float)dev_info->CF_Line) * 100));
...
static int jtag_cpld_lcm3d_verify(CPLDInfo* dev_info)
{
    ...
    operand = 0x000100;
    ast_jtag_tdi_xfer(JTAG_STATE_TLRESET, LCMXO3D_INIT_ADD_BITS_LEN, &operand);
    usleep(1000);
        ...
        case CHECK_BUSY:
            do
            {
                ret = i2c_rdwr_msg_transfer(
                    cpld.fd, cpld.slave << 1, busy_flag_cmd,
...
static int i2c_cpld_start()
{
    ...
    ret = i2c_rdwr_msg_transfer(cpld.fd, cpld.slave << 1, enable_program_cmd,
...
static int i2c_sendCFdata(CPLDInfo* dev_info)
{
    ...
    uint8_t program_page_cmd[1 + 3 + 16] = {
        0}; /* 1B Command + 3B Operands + 16B Write Data */
    int ret = 0;
    int CurrentAddr = 0;
    unsigned int i;
    unsigned status;
    ...
    for (i = 0; i < dev_info->CF_Line; i++)
    {
        ...
        ret = i2c_rdwr_msg_transfer(cpld.fd, cpld.slave << 1, program_page_cmd,
                                    1 + 3 + 16, NULL, 0);
        if (ret != 0)
        {
...
static int i2c_cpld_program(CPLDInfo* dev_info)
{
    ...
    ret = i2c_rdwr_msg_transfer(cpld.fd, cpld.slave << 1, prog_buf, 1 + 3 + 4,
                                NULL, 0);
    if (ret != 0)
    {
        ERR_PRINT("i2c_cpld_program(): Reset CFG Page Address");
...
static int yzbb_i2c_cpld_get_id(unsigned int* dev_id)
{
    ...
    uint8_t init_ufm_cmd[YZBB_INS_LENGTH] = {YZBB_INIT_ADRR_UFM, 0x00, 0x00,
                                             0x00};
    uint8_t ufm_cmd[YZBB_READ_UFM_INS_LENGTH] = {YZBB_READ_UFM_ADDR, 0x00, 0x00,
                                                 0x00};
