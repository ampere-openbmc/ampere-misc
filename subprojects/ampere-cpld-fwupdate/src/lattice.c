...
static int
jed_update_data_size(FILE *jed_fd, int *cf_size, int *ufm_size)
{
...
static int
jed_file_parser(FILE *jed_fd, CPLDInfo *dev_info, int cf_size, int ufm_size)
{
  /**TAG Information**/
...
//          printf("[%d]%x %x %x %x\n",dev_info->CF_Line, dev_info->CF[current_addr],dev_info->CF[current_addr+1],dev_info->CF[current_addr+2],dev_info->CF[current_addr+3]);
          ...
          for ( i = 0; i < sizeof(unsigned int); i++ )
          {
            ...
            printf("%x ", (dev_info->CF[current_addr+i]>>8) & 0xff);
            printf("%x ", (dev_info->CF[current_addr+i]>>16) & 0xff);
            printf("%x ", (dev_info->CF[current_addr+i]>>24) & 0xff);
          }
          printf("\n");
 #endif
...
static unsigned int
jtag_check_device_status(int mode)
{
  int RETRY = MAX_RETRY;
...
/*write cf data*/
static int
jtag_sendCFdata(CPLDInfo *dev_info)
{
  int ret = 0;
  int CurrentAddr = 0;
  unsigned int i;
...
static int
jtag_sendUFMdata(CPLDInfo *dev_info)
{
  int ret = 0;
  int CurrentAddr = 0;
  unsigned int i;
...
jtag_cpld_checksum(FILE *jed_fd, unsigned int *crc)
{
  ...
  *crc = 0;
  ...
  ret = jed_update_data_size(jed_fd, &cf_size, &ufm_size);
  if ( ret < 0 )
  {
...
static int
jtag_cpld_verify(CPLDInfo *dev_info)
{
  unsigned int i;
  int result;
  int current_addr = 0;
...
static int
jtag_cpld_erase(int erase_type)
{
  unsigned int dr_data[4] = {0};
  int ret = 0;
...
static int
jtag_cpld_program(CPLDInfo *dev_info)
{
  int ret = 0;
  unsigned int dr_data[4] = {0};
...
static unsigned int
i2c_check_device_status(int mode)
{
  int RETRY = MAX_RETRY;
  uint8_t flag[1];
  uint8_t status[4];
  ...
  switch (mode)
  {
    case CHECK_BUSY:
      do
      {
        ret = i2c_rdwr_msg_transfer(cpld.fd, cpld.slave << 1, busy_flag_cmd,
...
/*write cf data*/
static int
i2c_sendCFdata(CPLDInfo *dev_info)
{
  uint8_t write_page_cmd[4] = {0x70, 0x00, 0x00, 0x01};
  uint8_t program_page_cmd[1 + 3 + 16] = {0}; /* 1B Command + 3B Operands + 16B Write Data */
  int ret = 0;
  int CurrentAddr = 0;
  unsigned int i;
  unsigned status;
...
/*write ufm data if need*/
static int
i2c_sendUFMdata(CPLDInfo *dev_info)
{
  uint8_t write_page_cmd[4] = {0x70, 0x00, 0x00, 0x01};
  uint8_t program_page_cmd[1 + 3 + 16] = {0}; /* 1B Command + 3B Operands + 16B Write Data */
  int ret = 0;
...
static int
i2c_cpld_verify(CPLDInfo *dev_info)
{
  unsigned int i;
  int result;
  int current_addr = 0;
...
static int
i2c_cpld_program(CPLDInfo *dev_info)
{
  int ret = 0;
  unsigned int dr_data[4] = {0};
  uint8_t reset_addr_cmd[4] = {0x46, 0x00, 0x00, 0x00};
...
static int
LCMXO2Family_cpld_checksum(FILE *jed_fd, uint32_t *crc)
{
...
static int
LCMXO2Family_cpld_dev_open(cpld_intf_t intf, uint8_t id, cpld_intf_info_t *attr)
{
  int rc = 0;
  UNUSED(id);
...
static int
LCMXO2Family_cpld_dev_close(cpld_intf_t intf)
{
  if (intf == INTF_JTAG) {
    ast_jtag_close();
  } else if (intf == INTF_I2C) {
